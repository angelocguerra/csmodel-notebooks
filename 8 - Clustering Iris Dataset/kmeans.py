...
class KMeans(object):
    ...
    def adjust_centroids(self, data, groups):
        ...
        for i in range(self.k):
            # Filter the data to include only numeric columns for the current cluster
            cluster = data[groups == i].select_dtypes(include=[np.number])
            centroid = cluster.mean()
            new_centroids.append(centroid)

        # Convert the list of centroids into a DataFrame
        centroids = pd.DataFrame(new_centroids, columns=self.columns)
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            # data points.
            difference_groups = pd.Series(np.where(groups != cur_groups, 1, 0)).sum()
            flag_groups = (difference_groups == 0)

            # TODO: Check if there are changes with the values of the centroids
            difference_centroids = (self.centroids - centroids)
            flag_centroids = (difference_centroids == 0).all(axis = None)
